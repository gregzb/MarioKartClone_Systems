 ...
 *
 */
static void add_music(audio *root, audio *new);
 ...
 *
 */
static inline void play_audio(const char *filename, audio *audio, uint8_t loop, int volume);
 ...
 *
 */
static void add_audio(audio *root, audio *new);
 ...
 *
 */
static inline void audio_callback(void *userdata, uint8_t *stream, int len);

void play_sound(const char *filename, int volume)
{
    play_audio(filename, NULL, 0, volume);
}

void play_music(const char *filename, int volume)
{
    play_audio(filename, NULL, 1, volume);
}

void play_sound_from_memory(audio *audio, int volume)
{
    play_audio(NULL, audio, 0, volume);
}

void play_music_from_memory(audio *audio, int volume)
{
    play_audio(NULL, audio, 1, volume);
}
...
void init_audio(void)
{
    ...
    (gDevice->want).callback = &audio_callback;
...
void end_audio(void)
{
    if (gDevice->audioEnabled)
    {
        ...
        free_audio((audio *)(gDevice->want).userdata);
...
audio *create_audio(const char *filename, uint8_t loop, int volume)
{
...
static inline void play_audio(const char *filename, audio *audio, uint8_t loop, int volume)
{
    ...
    if (filename != NULL)
    {
        ...
        new = create_audio(filename, loop, volume);
    }
    else if (audio != NULL)
    {
        new = malloc(sizeof(struct audio));
        ...
        memcpy(new, audio, sizeof(struct audio));
    ...
    if (loop == 1)
    {
        add_music((struct audio *)(gDevice->want).userdata, new);
    }
    else
    {
        add_audio((struct audio *)(gDevice->want).userdata, new);
    }
...
static void add_audio(struct audio *root, struct audio *new)
{
    if (root == NULL)
    {
        return;
